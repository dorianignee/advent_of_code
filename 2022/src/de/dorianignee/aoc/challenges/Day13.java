...
public class Day13 extends Aoc {
    ...
    public int challenge2() {
        ...
        int divider2index = (int) items.stream()
                                       .filter(item -> divider2.compareTo(item) > 0)
                                       .count() + 1; // +1 because solution is 1-indexed

        int divider6index = (int) items.stream()
                                       .filter(item -> divider6.compareTo(item) > 0)
                                       .count() + 2; // +2 because solution is 1-indexed and divider2 is not in list
